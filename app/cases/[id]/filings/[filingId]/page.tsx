...
export default async function FilingDetailPage({ params }: { params: Promise<{ id: string; filingId: string }> }) {
  ...
  if (!caseData || !filing) return <><Disclaimer /><Header /><main className="wrap static-page"><p className="kicker">No local filing</p><h1>We couldn&apos;t find this demo filing.</h1><p>This prototype only opens filings bundled with its synthetic case data.</p><Link className="outline-cta" href={`/cases/${id}`}>Return to case →</Link></main><Footer /></>;
