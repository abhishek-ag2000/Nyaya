...
Use `/demo` before each replay. The canonical Tier-A case is `NYA-WB-DEMO-04821` (Sharma v. State of West Bengal); all data is local.

## A. Navigation
- [ ] Open `/`, `/login`, `/my-nyaya`, `/my-cases`, and the canonical case.
- [ ] Check `/cause-list`, `/filing-defects`, `/registry/filing-defects`, `/notifications`.
- [ ] Check Tier-C routes: live court, guide/article, certified copy, references, visit, help, judge, registry, police.
- [ ] Confirm all primary actions lead to a meaningful destination or clearly labelled demo state.

## B. Shared case state
- [ ] Reschedule hearing in demo controls; verify case header, overview, right rail, My Nyaya, My Cases, daily cause list, timeline and notification center.
...
## C. Core journey
- [ ] Login is mock-only and presents a demo identity.
- [ ] Upload flow reaches review, exposes extraction fields, requires content confirmation, and returns to the case.
...
## F. Compliance
- [ ] Persistent independent-project/demo-data disclaimer remains visible.
- [ ] No live government/API calls, official branding, real personal identifiers, payments, Aadhaar/PAN/OTP or real court records.
- [ ] Tier-C pages visibly say Preview; e-pass says not valid for entry; procedural tools retain their adjacent disclosures.
