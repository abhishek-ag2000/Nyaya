...
export default function NyayaAI() {
  ...
  const [messages, setMessages] = useState<Array<{ from: "user" | "ai"; text: string; action?: Reply["action"] }>>([
    { from: "ai", text: "Hello, I’m NyayaAI. I can help you find your way around this site." },
  ]);
  ...
  function ask(value: string) {
    ...
    setMessages((current) => [...current, { from: "user", text: prompt }, { from: "ai", ...reply }]);
    setQuestion("");
  }
  ...
  return (
    <div
      className={open ? "nyaya-ai open" : "nyaya-ai"}
    ...
    >
      <button
        ...
        aria-expanded={open}
        aria-controls="nyaya-ai-panel"
        aria-label={open ? "Close NyayaAI" : "Open NyayaAI"}
      >
      ...
      {open && (
        <section id="nyaya-ai-panel" className="ai-panel" aria-label="NyayaAI assistant">
          <div className="ai-panel-bg" aria-hidden="true" />
          <header className="ai-panel-header">
            <div className="ai-identity">
              <span className="ai-avatar" aria-hidden="true">
                <Bot />
                <Sparkles className="ai-avatar-spark" />
              </span>
              <span>
                <b>NyayaAI</b>
                <small>
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label="Close NyayaAI">
              <X aria-hidden="true" />
            </button>
          </header>

          <div className="ai-disclosure">
            Mock answers only. No live systems, personal data, or legal advice.
          </div>
          ...
          <div className="ai-messages" aria-live="polite" ref={messagesRef}>
            {messages.map((message, index) => (
              <div className={`ai-message ${message.from}`} key={`${message.from}-${index}`}>
                {message.from === "ai" && (
                  <span className="ai-message-avatar" aria-hidden="true"><Bot /></span>
                )}
                <div className="ai-message-body">
                  ...
                  {message.action && (
                    <button
                      type="button"
                      onClick={() => { router.push(message.action!.href); setOpen(false); }}
                    >
          ...
          <form className="ai-composer" onSubmit={submit}>
            <label className="sr-only" htmlFor="nyaya-ai-question">Ask NyayaAI</label>
            <input
              id="nyaya-ai-question"
              value={question}
