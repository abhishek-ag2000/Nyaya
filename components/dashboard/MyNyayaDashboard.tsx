...
import { getLatestCaseEvent, getOpenActionsForUser, getUpcomingItemsForUser, getUserCases } from "@/data/user-cases";
...
export default function MyNyayaDashboard() {
  ...
  if (!cases.length) return <main className="wrap dashboard-page"><p className="dashboard-loading">Loading your workspace…</p></main>;
  const actions = getOpenActionsForUser(cases);
  const upcoming = getUpcomingItemsForUser(cases);
  return (
    <main className="wrap dashboard-page">
      <p className="kicker">{role ? `${role} demo workspace` : "Demo workspace"}</p>
      <h1>Good evening{role ? `, ${role}.` : "."}</h1>
      <p className="dashboard-intro">Here&apos;s what needs your attention.</p>
      <p className="dashboard-summary">
        {cases.length} active cases <i /> {upcoming.filter((item) => item.type === "hearing").length} upcoming hearings <i />{" "}
      ...
      <section className="dashboard-section dashboard-attention">
        ...
        {actions.length ? (
            ...
            {actions.slice(0, 3).map(({ caseData, ...action }) => (
              <article key={action.id}>
                <div>
                  ...
                  {action.dueDate && <small>Due before: {date(action.dueDate)}</small>}
                </div>
                <Link href={`/cases/${caseData.id}`}>Review filing →</Link>
              </article>
            ))}
          </div>
        ) : (
          <p className="calm-empty">
            ...
            <small>Your tracked cases have no open action items.</small>
          </p>
        )}
        {actions.length > 3 && (
          <Link className="text-action" href="/my-cases?filter=attention">
            View all actions →
          </Link>
        )}
      </section>
        ...
        <div className="upcoming-list">
          {upcoming.slice(0, 4).map((item) => (
            <Link href={item.href} key={item.id}>
              ...
              <div>
                <b>{item.title}</b>
                <p>{item.type === "hearing" ? `Hearing · ${item.subtitle}` : `Due item · ${item.subtitle}`}</p>
              </div>
            </Link>
          ))}
        </div>
      </section>
      <section className="dashboard-section dashboard-cases">
        <div className="dashboard-heading">
          <span>My cases</span>
        ...
        <div>
          {cases.slice(0, 4).map((caseData) => (
            <CasePreview caseData={caseData} key={caseData.id} />
          ))}
        </div>
      </section>
    </main>
  ...
  return (
    <Link className="dashboard-case" href={`/cases/${caseData.id}`}>
      <div>
        <h2>{caseData.title}</h2>
        <p>
          {caseData.caseType} · {caseData.court.establishment}
        </p>
      </div>
      <p>
        <b>Stage</b>
      ...
      <p>
        <b>Next hearing</b>
        {date(caseData.nextHearing.date)} · {caseData.nextHearing.time}
      </p>
      <p className={count ? "has-attention" : ""}>
        <b>Attention</b>
        {count ? `${count} item` : "None"}
      </p>
      <span aria-label={`Open ${caseData.title}`}>→</span>
      {update && <small>Updated {date(update.occurredAt)}</small>}
    </Link>
  );
}
